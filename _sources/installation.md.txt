...
The PyG companion libraries are already declared in `environment.yml` and installed
by `conda env create`. If you ever need to reinstall them manually:
...
The pinned versions in `environment.yml` target **PyTorch 2.8 + CUDA 12.8**
(note the `+pt28cu128` suffix on the wheel filenames).

If `torch` is later upgraded (e.g., to 2.10), `import torch_geometric` will
**segfault** with no helpful error message. To verify versions are aligned:
...
or reinstall the PyG companion libs against your current torch version using the
matching index from <https://data.pyg.org/whl/>.
